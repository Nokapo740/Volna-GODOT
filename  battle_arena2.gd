...
func monster_attack() -> void:
	if hero.hp > 0:
		hero.hp -= 20  # Уменьшаем здоровье героя (например, 10 урона)
		update_ui()  # Обновляем интерфейс после получения урона

# Метод завершения боя
func end_battle() -> void:
	if hero.hp > 0:
		get_tree().change_scene_to_file("res://battle_arena_3.tscn")
	else:
...
func _on_attack_button_pressed() -> void:
	if is_hero_turn and buttons_enabled and not hero.is_attacking:
		...
		if randf() < 0.2:  # 20% шанс критического удара
			damage = randi() % 26 + 15  # Генерация случайного урона от 15 до 25 (включительно)
